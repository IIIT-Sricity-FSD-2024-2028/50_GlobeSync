...
function getDefaultNotifs(role) {
  const all = {
    ...
    admin: [
      { id: 1, type: 'warning', icon: '⚠️', title: 'New High Priority Ticket', desc: 'Ticket #101 — Hotel booking mismatch reported by Ajith Kumar.', time: '10 minutes ago', unread: true },
      { id: 2, type: 'success', icon: '✅', title: 'Booking Confirmed', desc: 'Trip to New York for Rahul Verma is now confirmed.', time: '30 minutes ago', unread: true },
      { id: 3, type: 'info', icon: '👥', title: 'New Traveler Registered', desc: 'Priya Sharma joined the platform and booked a trip.', time: '2 hours ago', unread: false },
    ...
    guide: [
      { id: 1, type: 'success', icon: '👤', title: 'New Traveler Assigned', desc: 'Ajith Kumar has been added to your traveler list for Paris trip.', time: '15 minutes ago', unread: true },
      { id: 2, type: 'message', icon: '💬', title: 'Message from Traveler', desc: 'Priya Sharma asked about the Swiss Alps tour schedule.', time: '1 hour ago', unread: true },
      { id: 3, type: 'info', icon: '📅', title: 'Trip Update', desc: "Rahul's New York trip has been confirmed. Prepare itinerary.", time: '3 hours ago', unread: false },
    ],
    support: [
      { id: 1, type: 'warning', icon: '🎫', title: 'New High Priority Ticket', desc: 'Ticket #103 — Payment not reflecting for Rahul Verma.', time: '5 minutes ago', unread: true },
      ...
      { id: 3, type: 'info', icon: '🔄', title: 'Refund Initiated', desc: 'Refund of ₹99,000 for Priya Sharma has been initiated.', time: '4 hours ago', unread: false },
    ],
  };
...
function loadNotifs(role) {
  const stored = sessionStorage.getItem(NOTIF_KEY + '_' + role);
...
function saveNotifs(role, notifs) {
  sessionStorage.setItem(NOTIF_KEY + '_' + role, JSON.stringify(notifs));
}
